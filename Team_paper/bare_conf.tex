...
Our plan is to perform complete \textit{Welcoming visitor}(referred as MW) task during competition. Moreover, we will work on \textit{Getting to know my home}(referred as MG) tasks, where we assume to be able to recognise state of doors. In order to achieve these goals, we set up our milestones:
\begin{itemize}
...
\item MW6 - \textbf{Perform complete \textit{Welcoming tasks} during British science festival 6.9.-11.9.2014}
...
He is a PhD student in the Intelligent Robotics lab of the School of Computer Science, University of Birmingham. He has a multidisciplinary background in control engineering, robotics and AI. His work on BARC involves but is not limited to control, navigation, localisation and in the future Human-Robot-Interaction (HRI).
...
For creating a state machine, we use ROS's SMACH package, that is ``a task-level architecture for rapidly creating complex robot behaviour" \cite{smach}. Our state machine can be seen in Fig. \ref{fig:smach} and it connect these nodes:
\begin{itemize}
